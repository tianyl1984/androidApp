...
public class PageWidget extends View {
	...
	private int mCornerX = 0; // 拖曳点对应的页脚X轴坐标
	private int mCornerY = 0;// 拖曳点对应的页脚Y轴坐标
	private Path mPath0;
	private Path mPath1;
	...
	PointF mBezierControl2 = new PointF();// 另一个贝塞尔曲线控制点
	PointF mBeziervertex2 = new PointF();// 另一个贝塞尔曲线顶点
	PointF mBezierEnd2 = new PointF();// 另一个贝塞尔曲线结束点

	float mMiddleX;// 直线中点X轴坐标
	float mMiddleY;// 直线中点Y轴坐标
	float mDegrees;// 卷起角度
	...
	float mMaxLength = (float) Math.hypot(mWidth, mHeight);// 最大斜边长度
	...
	public PageWidget(Activity context) {
		super(context);
		mPath0 = new Path();
		mPath1 = new Path();
		createDrawable();
